...
export async function generateQrDataUrl(value: string, size = 128): Promise<string> {
...
export async function generateBarcodeDataUrl(
  value: string,
  opts?: { format?: 'code128' | 'code39' | 'ean13'; width?: number; height?: number }
): Promise<string> {
  if (!value) return emptyPng(opts?.width ?? 200, opts?.height ?? 60);
  const { default: JsBarcode } = await import(/* webpackChunkName: "jsbarcode" */ 'jsbarcode');
