...
export async function listTemplatesAction(): Promise<
	{ id: string; name: string; createdAt: string; updatedAt: string; pages: number; fields: number }[]
> {
	const rows = await prisma.template.findMany({
		select: { id: true, name: true, createdAt: true, updatedAt: true, payload: true },
	...
	return rows.map((row) => {
		...
		const fields = pagesArr.reduce((acc, p) => acc + (Array.isArray(p.fields) ? p.fields.length : 0), 0);
		return {
			id: row.id,
			name: row.name,
...
export async function saveTemplateFromForm(formData: FormData) {
	...
	try {
		...
		if (typeof json !== "string") return { ok: false, error: "Missing template" } as const;
